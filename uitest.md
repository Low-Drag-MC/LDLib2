...
| `-PgraphicsBackend=vulkan` | pin the renderer, which is otherwise a video option that tries OpenGL first |
...
shard — as is `-PgraphicsBackend`, so a parallel run renders through the same backend as the serial
run it is reproducing; `-PldTestJobs` is the one that is required, because the shard runs are created
from it at configuration time.
