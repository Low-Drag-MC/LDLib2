...
public final class GraphViewPreferences {
    ...
    /** Resolved on first use, never in a static initialiser: {@link Platform} has no game path in a unit test. */
    @Nullable
    ...
    public static Path getFile() {
        if (file == null) {
            file = Platform.getGamePath().resolve("config").resolve(LDLib2.MOD_ID).resolve("graph_view.json");
        }
        return file;
    }
