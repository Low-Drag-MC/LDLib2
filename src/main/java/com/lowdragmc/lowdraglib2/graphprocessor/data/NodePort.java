...
import com.lowdragmc.lowdraglib2.graphprocessor.data.custom.ICustomPortIODelegate;
...
public class NodePort {
        ...
        if (!customPortIODelegateTable.contains(nodeClazz, fieldName)) {
            ...
            for (var method : methods) {
                ...
                if (customPortInput != null && customPortOutput != null) {
                    LDLib2.LOGGER.error("The method {} in the class {} is annotated with both CustomPortInput and CustomPortOutput, only one is allowed", method, nodeClazz);
                    continue;
                }
                var field = customPortInput != null ? customPortInput.field() : customPortOutput.field();
                if (field.equals(fieldName)) {
                    if (method.getParameterCount() != 2) {
                        LDLib2.LOGGER.error("The method {} in the class {} annotated with CustomPortInput or CustomPortOutput must have 2 parameters", method, nodeClazz);
                        continue;
                    }
                    if (!method.getParameterTypes()[0].isAssignableFrom(List.class)) {
                        LDLib2.LOGGER.error("The method {} in the class {} annotated with CustomPortInput or CustomPortOutput must have the second parameter of type List", method, nodeClazz);
                        continue;
                    }
                    if (!method.getParameterTypes()[1].isAssignableFrom(NodePort.class)) {
                        LDLib2.LOGGER.error("The method {} in the class {} annotated with CustomPortInput or CustomPortOutput must have the third parameter of type NodePort", method, nodeClazz);
                    }
                    ...
                    customPortIODelegateTable.put(nodeClazz, fieldName, (owner, edges, outputPort) -> {
                        try {
                            ...
                            LDLib2.LOGGER.error("Error while calling the method {} in the class {} annotated with CustomPortInput or CustomPortOutput", method, nodeClazz, e);
                        }
                    });
                    found = true;
                    break;
    ...
    public void PushData() {
        ...
        try {
            ...
            LDLib2.LOGGER.error("Error while getting the value of the field {} for remove custom IO", fieldInfo, e);
            return;
        }
    ...
    public void PullData() {
        ...
        if (!edges.isEmpty()) {
            ...
            try {
                ...
                LDLib2.LOGGER.error("Error while setting the value of the field {} with {} for pull data", fieldInfo, passThroughObject, e);
            }
        }
    }
    ...
    public void resetToDefault() {
        try {
            ...
            LDLib2.LOGGER.error("Error while resetting the value of the field {}", fieldInfo, e);
        }
    }
    ...
    public static class DefaultPushDataDelegate implements IPushDataDelegate {
        ...
        private Runnable createRunnable() {
            try {
                ...
                if (inType.isAssignableFrom(outType)) {
                    ...
                    return () -> {
                        try {
                            ...
                            LDLib2.LOGGER.error("Error while pushing data from {} to {}", edge.inputNode, edge.outputNode, e);
                        }
                    };
                ...
                LDLib2.LOGGER.error("Error while creating the push data delegate for edge the {}", edge, e);
                return Runnables.doNothing();
            }
            LDLib2.LOGGER.error("Error while creating the push data delegate for edge the {}", edge);
            return Runnables.doNothing();
        }
