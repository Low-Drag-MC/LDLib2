...
import com.lowdragmc.lowdraglib2.graphprocessor.data.parameter.ParameterNode;
import com.lowdragmc.lowdraglib2.graphprocessor.processor.BaseGraphProcessor;
import com.lowdragmc.lowdraglib2.graphprocessor.processor.TriggerProcessor;
import com.lowdragmc.lowdraglib2.gui.util.DrawerHelper;
import com.lowdragmc.lowdraglib2.gui.util.TreeBuilder;
import com.lowdragmc.lowdraglib2.gui.util.TreeNode;
import com.lowdragmc.lowdraglib2.gui.widget.DialogWidget;
import com.lowdragmc.lowdraglib2.gui.widget.FreeGraphView;
...
public class GraphViewWidget extends WidgetGroup {
    ...
    public GraphViewWidget(BaseGraph graph, int x, int y, int width, int height, @Nullable Consumer<List<String>> additionalGroup) {
        ...
        for (String group : supportNodeGroups) {
            for (var wrapper : LDLib2Registries.GRAPH_NODES.values()) {
    ...
    public void pasteTo(double mouseX, double mouseY) {
        ...
        for (var node : nodes) {
            ...
            newNode.position = Position.of((int) newPosition.x + offset.x, (int) newPosition.y + offset.y);
    ...
    public void addNode(BaseNode node, int x, int y) {
        ...
        node.position = Position.of(position.x, position.y);
        addNode(node);
    }
    ...
    public void addNodeToCenter(BaseNode node) {
        ...
        addNode(node, getSizeWidth() / 2 + LDLib2.RANDOM.nextInt(-20, 20),
                getSizeHeight() / 2 + LDLib2.RANDOM.nextInt(-20, 20));
    }
    ...
    private void drawGraphLines(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        for (var edge : graph.edges) {
            ...
            if (inputPort == null) {
                LDLib2.LOGGER.error("Edge {} input port {} are not existing: ", edge, edge.inputPort);
                continue;
            }
            if (outputPort == null) {
                LDLib2.LOGGER.error("Edge {} output port {} are not existing: ", edge, edge.outputPort);
                continue;
            }
