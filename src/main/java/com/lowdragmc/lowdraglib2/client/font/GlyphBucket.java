...
public final class GlyphBucket {
     ...
     */
    public static int select(Matrix4f pose) {
        ...
        var scaleX = (float) Math.sqrt(pose.m00() * pose.m00() + pose.m10() * pose.m10());
        var scaleY = (float) Math.sqrt(pose.m01() * pose.m01() + pose.m11() * pose.m11());
        ...
        if (mode == LDLibClientConfig.FontRenderMode.AUTO) {
            ...
            if (Math.abs(pose.m01()) > AXIS_EPSILON || Math.abs(pose.m10()) > AXIS_EPSILON) {
                return SDF;
            }
