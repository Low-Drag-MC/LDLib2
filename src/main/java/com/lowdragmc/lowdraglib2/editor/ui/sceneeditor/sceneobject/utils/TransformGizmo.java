...
public class TransformGizmo extends SceneObject implements ISceneRendering, ISceneInteractable {
    /**
     * Translucent, depth-test-off geometry, drawn in the order it is submitted.
     *
     * <p>Deliberately <b>not</b> {@code sortOnUpload()}. That sorts quads by distance from the origin
     * of the space the vertices were written in — which here is the scene's world space, not the
     * camera's — so it would reorder this geometry by a number that has nothing to do with what is in
     * front of what. The gizmo does not need it either: with the depth test off, back-to-front is
     * simply the order {@link #drawRotate} submits in, and that order is chosen for what it wants to
     * read like rather than left to a sort. {@link RenderUtils.ImmediateDraw} preserves it.
     */
     ...
     */
    private void drawDragAxisGuide(PoseStack poseStack, RenderUtils.ImmediateDraw bufferSource) {
     ...
     */
    private void drawBallOutline(PoseStack poseStack, RenderUtils.ImmediateDraw bufferSource) {
        if (targetTransform == null) return;
        var scale = getGizmoScale();
        var buffer = bufferSource.getBuffer(POSITION_COLOR_NO_DEPTH);
    ...
    private void drawScreenRing(PoseStack poseStack, RenderUtils.ImmediateDraw bufferSource) {
        if (targetTransform == null) return;
        var scale = getGizmoScale();
