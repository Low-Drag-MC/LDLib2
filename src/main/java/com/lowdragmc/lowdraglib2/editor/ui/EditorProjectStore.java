...
public final class EditorProjectStore {
    ...
    static List<RecentEntry> readEntries(CompoundTag tag) {
        ...
        var recent = tag.getListOrEmpty(RECENT_KEY);
        for (int i = 0; i < recent.size(); i++) {
            // both shapes in one pass: a list holds one element type, so exactly one of the two
            // accessors answers for any given file, and neither needs the element type up front
            var entry = recent.getCompound(i);
            if (entry.isPresent()) {
                var path = entry.get().getStringOr(PATH_KEY, "");
                if (!path.isEmpty()) {
                    entries.add(new RecentEntry(path, entry.get().getStringOr(TYPE_KEY, "")));
                }
                continue;
            }
            var legacy = recent.getStringOr(i, "");
            if (!legacy.isEmpty()) {
                entries.add(new RecentEntry(legacy, ""));
            }
        }
        return entries;
    }
