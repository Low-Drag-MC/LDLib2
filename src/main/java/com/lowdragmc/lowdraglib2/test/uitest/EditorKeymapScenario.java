...
public class EditorKeymapScenario implements UIScenario {
    ...
    private static Identifier themeLocation(String theme) {
    ...
    private static EditorAction action(TestContext ctx, Identifier id) {
        return editor(ctx).getKeymap().getAction(id).orElseThrow();
    }

    /** What the settings page currently has this action bound to. */
    private static KeyChord binding(TestContext ctx, Identifier id) {
