...
public class ModularUIWindow implements OsWindowHost {
    ...
    public void onDestroyed() {
        ...
        // A GuiRenderer is not garbage the collector can take: it holds an off-heap staged vertex
        // buffer, an item atlas texture, a projection ubo, a cube map, and a render target for every
        // picture-in-picture renderer it has pooled. And this host is not reused —
        // FloatingViewManager builds a new one each time a panel is floated out — so dropping it
        // here without closing leaks that whole set once per float.
        //
        // Safe to close precisely because ensureRenderer() gave it pools of its own: close() closes
        // the pools it holds, and the game renderer's are not among them. The one thing that is
        // borrowed — the feature dispatcher — close() never touches.
        if (renderer != null) {
            renderer.close();
            renderer = null;
        }
    ...
    public void renderFrame(float partialTick) {
        ...
        try (var ignoredSurface = UISurface.push(currentSurface);
             var ignoredActive = ModularUI.scopedActive(modularUI)) {
            // Built inside the scope, not before it. 26.2 seeds the scissor stack with a root
            // rectangle of guiWidth() x guiHeight() taken in the constructor, and every rectangle
            // pushed later is intersected with it (GuiGraphicsExtractor.ScissorStack#push). Read
            // against the game window, that root would clip this window's UI to the game window's
            // size — invisible while the game window is the larger of the two, and a UI cut off at
            // an arbitrary line as soon as it is not.
